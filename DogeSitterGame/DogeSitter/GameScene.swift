...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func createUniversalScene() {
        ...
        timerLabel.fontColor = UIColor.magenta
        ...
        gameTimer = Timer.scheduledTimer(timeInterval: 1.0, target: self, selector: #selector(self.updateTimer), userInfo: nil, repeats: true)
    }
    ...
    func createScoreTable() {
        if ((curLevel - 1) > (userDefaults.value(forKey: "highScore") as! Int)) {
            userDefaults.setValue(Int(curLevel - 1), forKey: "highScore")
        }
        
        scoreTable = SKSpriteNode(color: SKColor.white, size: CGSize(width: self.frame.width - 50, height: 300))
        ...
        highScoreLabel.text = "High Score: \(userDefaults.value(forKey: "highScore") as! Int)"
        highScoreLabel.fontColor = UIColor.magenta
        ...
        lastScoreLabel.fontColor = UIColor.magenta
        lastScoreLabel.position = CGPoint(x: self.frame.width / 2, y: self.frame.height / 2)
        lastScoreLabel.zPosition = 6
        
        restartButton = SKSpriteNode(color: SKColor.gray, size: CGSize(width: self.frame.width - 50, height: 100))
        ...
        restartButtonLabel.fontColor = UIColor.magenta
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
       /* Called when a touch begins */
        ...
        if (!gameOver) {
            ...
            if (curGame == 2) {
                if touch.location(in: self).y > buttonHeight {
                    gameTimer.invalidate()
                    gameOver = true
                    createScoreTable()
                }
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        if (!gameOver) {
            if (curGame == 1) {
                let touch = touches.first! as UITouch
                endTouchPoint = touch.location(in: self)
            ...
            if (curGame == 0) {
                if ((firstObject.categoryBitMask == PhysicsCategory.ground && secondObject.categoryBitMask == PhysicsCategory.toy) ||
                    ...
                    if (firstObject.categoryBitMask == PhysicsCategory.toy) {
                        firstObject.applyImpulse(CGVector(dx: 0, dy: 60))
                    } else {
                        secondObject.applyImpulse(CGVector(dx: 0, dy: 60))
                    }
                }
            } else if (curGame == 1) {
    ...
    func startBounceGame() {
        ...
        dogeSpriteForBounceGame.physicsBody?.isDynamic = true
    ...
    func updateDogeForBounceGame() {
        if (!gameOver) {
            if let roll = motionManager.deviceMotion?.attitude.roll {
                dogeSpriteForBounceGame.physicsBody?.velocity = CGVector(dx: CGFloat(roll) * 500, dy: 0)
            }
    ...
    func startBallTimer() {
        ballTimer = Timer.scheduledTimer(timeInterval: 1.0,
                                                           target: self,
    ...
    func startFrisbeeGame() {
        ...
        ceilingSprite.physicsBody = SKPhysicsBody(rectangleOf: ceilingSprite.size)
        ...
        dogeSpriteForFrisbeeGame.physicsBody = SKPhysicsBody(rectangleOf: dogeSpriteForFrisbeeGame.size)
        ...
        dogeSpriteForFrisbeeGame.physicsBody?.isDynamic = false
        ...
        frisbeeSprite.physicsBody?.isDynamic = false
        initialPos = CGPoint(x: self.size.width/2.0, y: 50)
        frisbeeSprite.physicsBody = SKPhysicsBody(rectangleOf: frisbeeSprite.size)
        ...
        desiredScoreLabel.fontColor = UIColor.magenta
    ...
    func moveSprite() {
        ...
        let moveRight = SKAction.moveBy(x: self.frame.size.width - 100, y: 0, duration: durationMovement)
        let moveLeft = SKAction.moveBy(x: -self.frame.size.width + 100, y: 0, duration: durationMovement)
        
        let moveBackAndForth = SKAction.repeatForever(SKAction.sequence([moveRight, moveLeft]))
        
        dogeSpriteForFrisbeeGame.run(moveBackAndForth)
    ...
    func startCatGame() {
        ...
        catScoreLabel.fontColor = UIColor.magenta
        ...
        catDesiredScoreLabel.fontColor = UIColor.magenta
